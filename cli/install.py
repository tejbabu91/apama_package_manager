...
def runInstall(packages_to_install):
	...
	for (n,v) in packages_to_install.items():
		if n in already_installed:
			if already_installed[n] == v.to_str():
	...
	print(f'The following packages are installed {", ".join(["%s:%s" % (k, v.to_str()) for (k, v) in packages_to_install.items()])}')